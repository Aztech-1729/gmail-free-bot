#!/usr/bin/env python3
"""IG signup v2 — REAL @gmail.com inbox (Emailnator XSRF-fixed) + IG chain."""
...
class GmailInbox:
    """REAL @gmail.com via Emailnator — XSRF-decoded client (proven Aug 2026)."""

    def __init__(self):
        self.s = cffi_requests.Session(impersonate="chrome")
        self.s.headers.update({"User-Agent": W})
        self.s.get("https://www.emailnator.com/", timeout=20)
        xsrf = urllib.parse.unquote(self.s.cookies.get("XSRF-TOKEN") or "")
        self.h = {"X-XSRF-TOKEN": xsrf, "X-Requested-With": "XMLHttpRequest",
    ...
    def create(self):
        r = self.s.post("https://www.emailnator.com/generate-email",
                        json={"email": ["dotGmail"]}, headers=self.h, timeout=30)
        if r.status_code != 200:
            return False
        e = r.json().get("email")
        e = e[0] if isinstance(e, list) else e
        if e and "gmail.com" in str(e):
            self.address = str(e)
            return True
        return False

    def poll_code(self, timeout=120):
        t0 = time.time()
        seen = set()
        while time.time() - t0 < timeout:
            try:
                r = self.s.post("https://www.emailnator.com/message-list",
                                json={"email": self.address}, headers=self.h, timeout=30)
                if r.status_code == 200:
                    data = r.json()
                    ...
                    for m in msgs:
                        ...
                        if not mid or mid in seen or mid == "ADSVPN":
                            continue
                        seen.add(mid)
                        r2 = self.s.post("https://www.emailnator.com/message-list",
...
def main():
    inbox = GmailInbox()
    if not inbox.create():
        print(json.dumps({"ok": False, "error": "gmail creation failed"}))
        return
    print(json.dumps({"step": "gmail", "addr": inbox.address}))
    for ver, code in VERSIONS:
        ...
        sn = base64.encodebytes(f"{inbox.address}|{ts}|".encode() + secrets.token_bytes(24)).decode().strip()
        data = {
            ...
            "email": inbox.address, "force_sign_up_code": str(vcode),
        ...
        if r2.status_code == 200:
            ck = dict(s.cookies)
            out = {"ok": True, "email": inbox.address, "username": username,
